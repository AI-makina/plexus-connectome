...
export default function EngineUpdateBadge({ plexus }: { plexus: any }) {
    ...
    const hasUpdate = update || vendorPending;
    ...
    // The version an update would install: what the vendor queued, else the fresh on-disk build.
    const targetVer: string | null = pending?.target_version
        || (v.latest_version && v.latest_version !== v.version ? v.latest_version : null);
                        ...
                        {checking ? (
                            ...
                            <>
                                <div className="flex items-center justify-between">
                                    <span className="text-[12px]" style={{ color: hasUpdate ? '#7BD6A0' : 'var(--text-mid)' }}>
                                        {hasUpdate ? `Update available${targetVer ? ` → v${targetVer}` : ''}` : '✓ On the latest'}
                                    </span>
                                    {(hasUpdate || latest?.notes?.length) ? (
                                ...
                                {showNotes ? (
                                    <div className="mt-2 border-t border-line pt-2">
                                        {targetVer ? (
                                            <div className="readout text-[10px] text-text-mid">Updating to <span style={{ color: '#7BD6A0' }}>v{targetVer}</span>{latest?.title ? ` — ${latest.title}` : ''}</div>
                                        ) : null}
                                        {latest?.notes?.length ? (
                                            <ul className={`space-y-1 ${targetVer ? 'mt-1.5' : ''}`}>
                                        ...
                                        {!targetVer && !latest?.notes?.length ? (
                                            <div className="text-[10px] text-text-lo">You're on the latest version.</div>
                                        ) : null}
                                    </div>
                                ) : null}
                                {update && (
                                    <button onClick={apply} disabled={applying}
                                        ...
                                        {applying ? 'Applying — reconnecting…' : (targetVer ? `Apply update → v${targetVer}` : 'Apply update')}
                                    </button>
                                )}
                                <button onClick={scan}
