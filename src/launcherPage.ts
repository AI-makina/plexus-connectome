...
<div class="modal" id="aiguide-modal" onclick="if(event.target===this)closeAiGuide()">
  <div class="modal-card guide">
    ...
    <div class="g-item"><span class="n">3</span><span><b>Codex CLI — global-only, connected by YOUR hands.</b> Codex only supports a global, always-on connection (OpenAI's design), and Plexus <b>never connects any AI globally</b>. So when you tap Codex in "Open project", Plexus shows you a statement and a command — <b>you</b> copy it and run it in any terminal, once per machine. From then on Codex is fully automatic in every Plexus project (and dormant everywhere else), and it wears a <b>global</b> tag so the exception stays visible. <b>Disengage</b> — in <span class="golink" onclick="closeAiGuide();openTools()">Manage connections</span> — removes Plexus from Codex on one click — removal only ever shrinks Plexus's reach, which is why a button may do it; reconnecting later is the same one-time paste.</span></div>
